...
// ─── Config ────────────────────────────────────────────────────────────────────
const DATA_DIR = path.resolve(process.env.MCP_DATA_DIR ?? path.join(os.homedir(), '.mcp-agent'));
const ROOT_SEP = IS_WIN ? ';' : ':';
const ALLOWED_ROOTS = (process.env.MCP_ALLOWED_ROOTS ?? os.homedir())
    .split(ROOT_SEP)
    .filter(Boolean)
...
async function ps(script, timeoutMs = 15_000) {
    if (!IS_WIN)
        throw new Error('PowerShell is only available on Windows.');
    const { stdout, stderr } = await execFileAsync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], { timeout: timeoutMs });
    return (stdout || stderr || 'ok').trim();
}
async function shell(cmd, timeoutMs = 15_000) {
...
async function openApp(target) {
    if (IS_WIN)
        await ps(`Start-Process -FilePath ${JSON.stringify(target)}`);
    else if (IS_MAC)
        await execFileAsync('open', ['-a', target]);
    else
        await execFileAsync('nohup', [target], {});
}
...
function addRecord(store, fields) {
...
function updateRecord(store, id, patch) {
    const records = readStore(store);
...
// ─── File Search ───────────────────────────────────────────────────────────────
async function walkSearch(dir, term, depth, results) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory())
            await walkSearch(full, term, depth + 1, results);
    }
}
// ─── App finder (Windows) ──────────────────────────────────────────────────────
const WIN_APP_ROOTS = IS_WIN ? [
 ...
 */
async function openSpotifyUri(uri) {
    if (IS_WIN)
        await ps(`Start-Process -FilePath ${JSON.stringify(uri)}`);
    else if (IS_MAC)
...
// YOUTUBE HELPERS
// ══════════════════════════════════════════════════════════════════════════════
async function youtubeSearchFirstId(query) {
...
'@
`;
async function winGetVolume() {
    const out = await ps(`${WIN_VOLUME_CS}\n[AudioDevice]::GetVolume()`);
    return parseInt(out.trim(), 10);
}
async function winSetVolume(pct) {
...
// SYSTEM TOOLS
// ══════════════════════════════════════════════════════════════════════════════
server.tool('system_info', 'Return hardware, OS, memory, and agent configuration for this machine.', {}, async () => {
    try {
        const cpus = os.cpus();
...
server.tool('list_processes', 'List the top running processes sorted by CPU usage.', {}, async () => {
    try {
        let out;
        if (IS_WIN) {
            out = await ps('Get-Process | Sort-Object CPU -Descending | ' +
...
// ── Disk Usage ──────────────────────────────────────────────────────────────
server.tool('get_disk_usage', 'Report disk space usage for all drives/partitions.', {}, async () => {
    try {
        let out;
        if (IS_WIN) {
            out = await ps('Get-PSDrive -PSProvider FileSystem | ' +
                'Select-Object Name,' +
...
// ── Battery ─────────────────────────────────────────────────────────────────
server.tool('get_battery', 'Get battery status: charge level, charging state, and estimated time remaining.', {}, async () => {
    try {
        let out;
        if (IS_WIN) {
            out = await ps('Get-WmiObject Win32_Battery | ' +
...
// ── Network Info ─────────────────────────────────────────────────────────────
server.tool('get_network_info', 'List network adapters, IP addresses, and current Wi-Fi SSID.', {}, async () => {
    try {
        const lines = [];
        ...
        try {
            let ssid;
            if (IS_WIN)
                ssid = await ps('(netsh wlan show interfaces) -match "\\s+SSID\\s+:" | ForEach-Object { $_ -replace ".*:\\s+","" }');
            else if (IS_MAC)
                ssid = await shell('/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport -I | grep " SSID:" | awk \'{print $2}\'');
            else
                ssid = await shell('iwgetid -r 2>/dev/null || nmcli -t -f active,ssid dev wifi 2>/dev/null | grep "^yes" | cut -d: -f2');
...
// ── Volume ──────────────────────────────────────────────────────────────────
server.tool('get_volume', 'Get the current system master volume level (0–100).', {}, async () => {
    try {
        let vol;
        if (IS_WIN) {
...
server.tool('set_volume', 'Set the system master volume to a specific level (0–100). Works on Windows, macOS, and Linux.', {
...
// ── Screenshot ───────────────────────────────────────────────────────────────
server.tool('take_screenshot', 'Capture the primary screen and save the image to disk. Returns the saved file path.', {
    savePath: z.string().optional().describe('Absolute path to save the PNG (default: Desktop/screenshot-<timestamp>.png)'),
}, async ({ savePath }) => {
    try {
...
`;
            ...
            try {
                ...
                try {
                    await execFileAsync('gnome-screenshot', ['-f', outFile]);
                }
                catch {
                    await shell(`import -window root ${JSON.stringify(outFile)} 2>/dev/null`);
                }
            }
        }
        if (!fs.existsSync(outFile))
            return fail('Screenshot file was not created. Ensure a display is connected.');
...
// ══════════════════════════════════════════════════════════════════════════════
server.tool('optimize_pc', 'Run a comprehensive PC optimization/cleanup: clears temp files, flushes DNS, empties recycle bin, ' +
    ...
    aggressiveness: z.enum(['light', 'full']).default('light').describe('light = safe cleanup only | full = also clears browser caches and Windows Update cache'),
}, async ({ aggressiveness }) => {
    const report = [`PC Optimization Report — ${new Date().toLocaleString()}`, ''];
    // 1. Clear temp files
    try {
        const tempDirs = [os.tmpdir()];
        if (IS_WIN) {
            tempDirs.push(path.join(os.homedir(), 'AppData', 'Local', 'Temp'), 'C:\\Windows\\Temp');
            if (aggressiveness === 'full') {
                tempDirs.push(path.join(os.homedir(), 'AppData', 'Local', 'Microsoft', 'Windows', 'INetCache'));
            }
        }
        else if (IS_MAC) {
        ...
        report.push(`✗ Temp cleanup failed: ${e instanceof Error ? e.message : e}`);
    }
    // 2. Flush DNS
    try {
        if (IS_WIN)
            ...
            await shell('sudo dscacheutil -flushcache; sudo killall -HUP mDNSResponder');
        else
            await shell('sudo systemctl restart systemd-resolved 2>/dev/null || sudo service dns-clean restart 2>/dev/null');
        report.push('✓ DNS cache flushed');
    }
    catch {
        report.push('⚠ DNS flush skipped (may need elevated privileges)');
    }
    ...
    try {
        ...
        report.push('⚠ Could not empty Recycle Bin / Trash');
    }
    // 4. Windows Update cache (full mode only)
    if (IS_WIN && aggressiveness === 'full') {
        try {
            await ps('Stop-Service wuauserv -Force -ErrorAction SilentlyContinue; ' +
            ...
            report.push('⚠ Windows Update cache clear skipped (needs admin)');
        }
    }
    ...
    try {
        let memInfo;
        if (IS_WIN) {
            memInfo = await ps('Get-Process | Sort-Object WorkingSet -Descending | Select-Object -First 5 ' +
        ...
        report.push('⚠ Could not read process list');
    }
    // 6. Current disk space after cleanup
    try {
        let diskInfo;
        if (IS_WIN) {
            diskInfo = await ps('Get-PSDrive -PSProvider FileSystem | ' +
...
server.tool('clear_temp_files', 'Delete temporary files from system and user temp directories. Returns count and size freed.', {}, async () => {
    try {
...
server.tool('empty_recycle_bin', 'Empty the Recycle Bin (Windows) or Trash (macOS/Linux).', {}, async () => {
    try {
        if (IS_WIN) {
            await ps('Clear-RecycleBin -Force -ErrorAction SilentlyContinue');
        }
...
server.tool('flush_dns', 'Flush the DNS resolver cache. May require elevated privileges on some systems.', {}, async () => {
    try {
        if (IS_WIN)
            await ps('Clear-DnsClientCache');
        else if (IS_MAC)
            ...
            await shell('sudo systemd-resolve --flush-caches 2>/dev/null || sudo service dns-clean restart');
...
server.tool('check_startup_apps', 'List applications that launch at startup (Windows registry + startup folder; macOS launchctl; Linux systemctl).', {}, async () => {
    try {
        let out;
        if (IS_WIN) {
            out = await ps('$keys = @(' +
...
server.tool('disk_health_check', 'Run a read-only disk health check. On Windows: SMART status via WMI. On Mac: diskutil. On Linux: smartctl.', {}, async () => {
    try {
        let out;
        if (IS_WIN) {
            out = await ps('Get-WmiObject -Namespace root\\wmi -Class MSStorageDriver_FailurePredictStatus | ' +
                'Select-Object InstanceName,' +
                '@{N="PredictFailure";E={$_.PredictFailure}},' +
                '@{N="Reason";E={$_.Reason}} | ConvertTo-Json', 30_000);
        }
        else if (IS_MAC) {
...
// APP / BROWSER TOOLS
// ══════════════════════════════════════════════════════════════════════════════
server.tool('find_app', 'Search common install locations for an application and return its full path. Windows only.', {
    name: z.string().min(1).max(100).describe('App name without .exe, e.g. "spotify", "discord"'),
}, async ({ name }) => {
    try {
        if (!IS_WIN)
            return fail('find_app is only available on Windows. On Mac/Linux use the app name directly with open_app.');
...
server.tool('open_app', 'Launch an application by name or full path. ' +
...
server.tool('close_app', 'Force-terminate a running process by its executable name.', {
...
server.tool('open_url', 'Open a URL in the default browser.', { url: z.string().url().describe('Full URL including https://') }, async ({ url }) => {
    try {
        await openURL(url);
...
server.tool('browser_search', 'Open a Google search in the default browser.', { query: z.string().min(1).max(500).describe('Search query') }, async ({ query }) => {
    try {
...
// SPOTIFY TOOLS
// ══════════════════════════════════════════════════════════════════════════════
server.tool('spotify_search', 'Search Spotify for tracks, artists, or albums and return results. ' +
    ...
    try {
        const token = await getSpotifyToken();
        if (!token) {
            return ok(`Spotify Web API is not configured.\n` +
        ...
        const res = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
        if (!res.ok)
            return fail(`Spotify API error: ${res.status}`);
        const data = await res.json();
        const items = data?.tracks?.items ?? data?.artists?.items ?? data?.albums?.items ?? [];
        if (!items.length)
            return ok(`No ${type} results found for "${query}".`);
        const lines = [`Spotify results for "${query}":\n`];
...
server.tool('spotify_play', 'Play music on the installed Spotify desktop app. ' +
    ...
    query: z.string().min(1).max(300).describe('Song/artist/album name OR a Spotify URI (spotify:track:xxx etc.)'),
    ...
    try {
        ...
        if (token) {
            ...
            if (tracks && tracks.length > 0) {
                ...
                return ok(`▶ Now playing on Spotify:\n` +
                    `  Track:  ${t.name}\n` +
                    `  Artist: ${t.artists}\n` +
                    `  Album:  ${t.album}\n` +
                    `  URI:    ${t.uri}`);
        ...
        return ok(`▶ Opened Spotify with search: "${query}"\n` +
...
// YOUTUBE TOOLS
// ══════════════════════════════════════════════════════════════════════════════
server.tool('youtube_search', 'Search YouTube and return the top video results (title, channel, URL) without opening a browser.', {
    ...
    try {
        ...
        if (!res.ok)
            return fail(`YouTube request failed: ${res.status}`);
        const html = await res.text();
        const match = html.match(/var ytInitialData\s*=\s*(\{.+?\});\s*<\/script>/s);
        if (!match)
            return fail('Could not parse YouTube response.');
        const data = JSON.parse(match[1]);
        const contents = data?.contents?.twoColumnSearchResultsRenderer
            ?.primaryContents?.sectionListRenderer
            ?.contents?.[0]?.itemSectionRenderer?.contents ?? [];
        ...
        if (!results.length)
            return ok('No video results found.');
...
server.tool('youtube_play', 'Search YouTube for a video and open it playing in the default browser immediately. ' +
...
// EMAIL
// ══════════════════════════════════════════════════════════════════════════════
server.tool('send_email', 'Open the default mail client with a pre-filled draft via mailto: link.', {
...
// CLIPBOARD
// ══════════════════════════════════════════════════════════════════════════════
server.tool('clipboard_get', 'Read the current text content of the system clipboard.', {}, async () => {
    try {
        let text;
        if (IS_WIN)
            text = await ps('Get-Clipboard');
        else if (IS_MAC)
            text = await shell('pbpaste');
        else
            text = await shell('xclip -selection clipboard -o 2>/dev/null || xsel --clipboard --output 2>/dev/null || echo ""');
...
server.tool('clipboard_set', 'Write text to the system clipboard.', { text: z.string().max(50_000).describe('Text to copy to clipboard') }, async ({ text }) => {
    try {
        const tmp = path.join(os.tmpdir(), `mcp-clip-${randomUUID()}.txt`);
        fs.writeFileSync(tmp, text, 'utf8');
        try {
            if (IS_WIN)
                await ps(`Get-Content -Raw ${JSON.stringify(tmp)} | Set-Clipboard`);
            else if (IS_MAC)
                await shell(`cat ${JSON.stringify(tmp)} | pbcopy`);
            else
                await shell(`cat ${JSON.stringify(tmp)} | xclip -selection clipboard 2>/dev/null || cat ${JSON.stringify(tmp)} | xsel --clipboard --input`);
...
// FILE TOOLS
// ══════════════════════════════════════════════════════════════════════════════
server.tool('list_dir', 'List the contents of a directory (files and subdirectories).', {
...
server.tool('search_files', `Search for files whose name contains a term. Searches up to ${MAX_SEARCH_DEPTH} levels deep.`, {
    ...
    try {
        const safe = safePath(dir);
        if (!fs.existsSync(safe))
            return fail(`Directory not found: ${safe}`);
        if (!fs.statSync(safe).isDirectory())
            return fail('Path is not a directory.');
        const results = [];
...
server.tool('read_file', `Read a file's text content. Returns up to ${MAX_FILE_READ_BYTES / 1024} KB.`, {
...
server.tool('write_file', 'Write content to a file (creates or overwrites). Creates parent directories if needed.', {
    ...
    try {
        const safe = safePath(filepath);
        if (!overwrite && fs.existsSync(safe))
...
server.tool('append_file', 'Append text to the end of a file. Creates the file if it does not exist.', {
    filepath: z.string().describe('Absolute path to the file'),
...
server.tool('copy_file', 'Copy a file to a new location.', {
    ...
    try {
        ...
        if (!overwrite && fs.existsSync(safeDest))
            return fail('Destination exists. Set overwrite=true to replace.');
        fs.mkdirSync(path.dirname(safeDest), { recursive: true });
...
server.tool('delete_file', 'Permanently delete a single file (not a directory).', { filepath: z.string().describe('Absolute path to the file to delete') }, async ({ filepath }) => {
    try {
        const safe = safePath(filepath);
        if (!fs.existsSync(safe))
            return fail('File not found.');
        if (fs.statSync(safe).isDirectory())
            return fail('Path is a directory. Use delete_file only for files.');
        fs.unlinkSync(safe);
...
server.tool('organize_folder', 'Sort all files in a folder into sub-folders named by their extension (e.g. PDF/, PNG/, TXT/).', { dir: z.string().describe('Absolute path to the folder to organize') }, async ({ dir }) => {
    try {
        const safe = safePath(dir);
        if (!fs.existsSync(safe))
            return fail('Directory not found.');
        ...
        if (errors.length)
            lines.push('', `Skipped ${errors.length} item(s):`, ...errors);
        return ok(lines.join('\n'));
    }
    catch (e) {
        return fail(e);
...
server.tool('create_task', 'Create a new to-do task with an optional priority.', {
    ...
    try {
        const task = addRecord('tasks', { title, status: 'todo', priority });
...
server.tool('list_tasks', 'List tasks, optionally filtered by status.', { status: z.enum(['todo', 'done', 'all']).default('all') }, async ({ status }) => {
    try {
        const tasks = readStore('tasks');
        const shown = status === 'all' ? tasks : tasks.filter(t => t.status === status);
        if (!shown.length)
            return ok(`No ${status === 'all' ? '' : status + ' '}tasks found.`);
...
server.tool('complete_task', 'Mark a task as done by its ID.', { id: z.string().uuid() }, async ({ id }) => {
    try {
        const updated = updateRecord('tasks', id, { status: 'done' });
...
server.tool('delete_task', 'Permanently delete a task by its ID.', { id: z.string().uuid() }, async ({ id }) => {
    try {
...
server.tool('create_note', 'Create a new note with a title, body, and optional tags.', {
    ...
    try {
        const note = addRecord('notes', { title, body, tags });
...
server.tool('list_notes', 'List all saved notes (titles, IDs, and tags).', { tag: z.string().optional() }, async ({ tag }) => {
    try {
        let notes = readStore('notes');
        if (tag)
            notes = notes.filter(n => n.tags?.includes(tag));
        if (!notes.length)
            return ok(tag ? `No notes with tag '${tag}'.` : 'No notes found.');
        return ok(notes.map(n => `${n.id.slice(0, 8)}  ${n.title}  [${(n.tags ?? []).join(', ') || 'no tags'}]  (${n.createdAt.slice(0, 10)})`).join('\n'));
    }
    catch (e) {
        return fail(e);
    }
});
server.tool('read_note', 'Read the full content of a note by its ID.', { id: z.string().uuid() }, async ({ id }) => {
    try {
        const note = readStore('notes').find(n => n.id === id);
        if (!note)
            return fail(`No note found with ID: ${id}`);
...
server.tool('search_notes', 'Search note titles and bodies for a keyword.', { query: z.string().min(1).max(200) }, async ({ query }) => {
    try {
        const lower = query.toLowerCase();
        const hits = readStore('notes').filter(n => n.title.toLowerCase().includes(lower) || n.body.toLowerCase().includes(lower));
        if (!hits.length)
...
server.tool('delete_note', 'Permanently delete a note by its ID.', { id: z.string().uuid() }, async ({ id }) => {
    try {
...
server.tool('set_reminder', 'Save a reminder with a title and an ISO 8601 datetime.', {
    ...
    try {
        if (isNaN(Date.parse(remindAt)))
            return fail('Invalid datetime. Use ISO 8601: 2025-06-01T09:00:00');
        const r = addRecord('reminders', { title, remindAt, done: false });
...
server.tool('list_reminders', 'List reminders sorted by time.', {
    ...
    try {
        let reminders = readStore('reminders');
        if (!includeDone)
...
server.tool('complete_reminder', 'Mark a reminder as done by its ID.', { id: z.string().uuid() }, async ({ id }) => {
    try {
        const updated = updateRecord('reminders', id, { done: true });
...
server.tool('delete_reminder', 'Delete a reminder by its ID.', { id: z.string().uuid() }, async ({ id }) => {
    try {
...
async function main() {
    ...
    process.stderr.write(`[local-pc-agent-pro v5] Ready\n` +
        `  Platform:      ${os.platform()}\n` +
        `  Data dir:      ${DATA_DIR}\n` +
        `  Allowed roots: ${ALLOWED_ROOTS.join(', ')}\n` +
