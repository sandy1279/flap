...
function assertOk(name: string, text: string, snippet?: string) {
...
function assertFails(name: string, text: string) {
  const isFailure =
...
async function runTests(client: Client) {
  ...
  if (names.length >= 20) pass('tool count', `${names.length} tools registered`);
  else fail('tool count', `Only ${names.length} tools found — server may not have started correctly`);
  ...
  if (!taskId) fail('create_task — could not parse task ID', text);
  else pass('create_task — ID parsed', taskId);
  ...
  if (!noteId) fail('create_note — could not parse note ID', text);
  else pass('create_note — ID parsed', noteId);
  ...
  if (!remId) fail('set_reminder — could not parse ID', text);
  else pass('set_reminder — ID parsed', remId);
