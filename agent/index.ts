...
function parseServerConfigs(): ServerConfig[] {
  if (process.env.MCP_SERVERS) {
    let parsed: unknown;
    try { parsed = JSON.parse(process.env.MCP_SERVERS); }
    catch { throw new Error("MCP_SERVERS must be valid JSON. Example:\n  '[{\"name\":\"local\",\"command\":\"node\",\"args\":[\"./tools/dist/index.js\"]}]'"); }
...
class AsyncLogger {
  ...
  info  = (m: string, meta?: unknown) => this.write("INFO",  m, meta);
  warn  = (m: string, meta?: unknown) => this.write("WARN",  m, meta);
  error = (m: string, meta?: unknown) => this.write("ERROR", m, meta);
  tool  = (m: string, meta?: unknown) => this.write("TOOL",  m, meta);
...
/** A tool registered in the global registry, with source info. */
interface RegisteredTool {
  ollamaName: string;    // name Ollama sees (may be "server__tool" if collision)
  mcpName:    string;    // original MCP tool name
  serverName: string;    // human-readable source server
  client:     Client;    // which client to call
  schema:     OllamaTool;
}

/** Events emitted by the agent loop (used for SSE and CLI). */
type AgentEvent =
  | { type: "tool_start";  tool: string; args: Record<string, unknown> }
  | { type: "tool_done";   tool: string; result: string; elapsedMs: number }
  | { type: "reply";       content: string }
  | { type: "done";        trace: string[] }
...
function mcpToOllamaTool(t: McpTool, ollamaName: string): OllamaTool {
...
/* ══════════════════════════════════════════════════════════════════════════ */
/* TOOL REGISTRY  (multi-server, collision-safe)                              */
/* ══════════════════════════════════════════════════════════════════════════ */
...
function buildToolRegistry(conns: ServerConnection[]): Map<string, RegisteredTool> {
  ...
  if (registry.size === 0) logger.warn("No tools were loaded from any MCP server.");
  return registry;
}
...
class SessionStore {
  ...
  private evict(): void {
    ...
    for (const [id, s] of this.store)
      if (s.lastActive < cutoff) { this.store.delete(id); logger.info(`Session evicted: ${id}`); }
  }
  ...
  for (let loop = 0; loop < CONFIG.MAX_LOOPS; loop++) {
      ...
      toolCalls.map(async tc => {
        ...
        try {
          const raw = await withTimeout(
            reg.client.callTool({ name: reg.mcpName, arguments: args }),
            CONFIG.TOOL_TIMEOUT,
            toolName,
          );
          const block = (raw.content as { type: string; text?: string }[])[0];
    ...
    for (const r of results) {
      const text = r.status === "fulfilled"
        ? r.value.result
        : `ERROR: ${(r.reason as Error)?.message ?? r.reason}`;
...
function buildHttpServer(
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (!res.headersSent) json(res, 500, { error: "Internal server error", requestId: rid });
    } finally {
      inFlight--;
    }
  });

  async function route(req: http.IncomingMessage, res: http.ServerResponse, rid: string) {
    const url    = req.url ?? "/";
    ...
    if (method === "POST" && url === "/chat") {
      ...
      if (!input) return json(res, 400, { error: "Missing 'message' field" });
  ...
  async function shutdown(): Promise<void> {
    ...
    if (inFlight > 0) logger.warn(`Force-closing with ${inFlight} request(s) still active`);
...
async function runCLI(registry: Map<string, RegisteredTool>): Promise<void> {
  const rl = createInterface({ input: process.stdin, output: process.stdout });

  // Persistent history across all CLI turns
  const history: OllamaMessage[] = [{ role: "system", content: SYSTEM_PROMPT }];

  console.log(`\n🤖  local-mcp-agent v5 — ${registry.size} tools loaded`);
  console.log(`    Model: ${CONFIG.MODEL}   Type "exit" to quit.\n`);

  for (;;) {
    const input = await new Promise<string>(r => rl.question("You: ", r));
...
async function main(): Promise<void> {
  ...
  const connections: ServerConnection[] = await Promise.all(
    CONFIG.SERVERS.map(async (cfg): Promise<ServerConnection> => {
      ...
      const client = new Client(
        { name: "local-mcp-agent", version: "5.0.0" },
        { capabilities: {} },
      );
